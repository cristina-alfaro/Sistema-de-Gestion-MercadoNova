...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Validar que productos existen y están activos
        ...
        BEGIN
            RAISERROR('Error: Uno o más productos no existen o están inactivos', 16, 1);
            ROLLBACK TRANSACTION;
            RETURN;
        END
        ...
        -- INSERTAR VENTA (el trigger TR_CalcularTotalVenta calculará el total automáticamente)
        ...
        -- Auditoría manual (el trigger de stock negativo hará auditoría si es necesario)
...
BEGIN
    ...
    BEGIN
        RAISERROR('Error: Ya existe un cliente registrado con este número de DUI.', 16, 1);
        RETURN;
    END
...
BEGIN
    ...
    BEGIN
        RAISERROR('Error: El precio mínimo no puede ser mayor al precio máximo', 16, 1);
        RETURN;
    END
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Validar que el JSON de productos no esté vacío
        IF @productos_json IS NULL OR LTRIM(RTRIM(@productos_json)) = '' OR @productos_json = '[]'
        BEGIN
        ...
        BEGIN
            RAISERROR('Error: Uno o más proveedores no existen', 16, 1);
            ROLLBACK TRANSACTION;
            RETURN;
        END
        ...
        BEGIN
            RAISERROR('Error: Uno o más productos no pertenecen al proveedor especificado', 16, 1);
            ROLLBACK TRANSACTION;
            RETURN;
        END
        ...
        -- Registrar en auditoría de inventario (usando @id_sucursal_empleado)
        INSERT INTO AuditoriaInventario (
            id_producto, 
            id_sucursal, 
            fecha, 
        ...
        -- Retornar resumen general de la compra (incluyendo la sucursal automática)
        SELECT 
            @total_compra AS total_compra_general,
            @nombre_gerente AS gerente_responsable,
            @id_sucursal_empleado AS id_sucursal,  -- Informamos qué sucursal se usó
